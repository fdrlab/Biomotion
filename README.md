...
This repository contains the Python analysis code and supporting data files required to reproduce the results of the currently unpublished study titled "Acute lorazepam effects on social interaction processing in 7T fMRI", by Niko Britt, Nahid Hasan, and Johannes Schultz, which investigated the effects of acute lorazepam administration on neural representations of social biological motion using 7T fMRI. The full fMRI dataset can be found on OpenNeuro (Link: __https://openneuro.org/datasets/ds007953__).
